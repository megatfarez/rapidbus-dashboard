...
st.set_page_config(page_title="RapidBus Bunching Detection by UniKL", layout="wide")
...
def render_bunching_strip(a, b, threshold_m=200.0):
    ...
    <div style="display:flex;justify-content:space-between;font-size:13px;color:#fff;background:#222;padding:6px 10px;border-radius:8px;">
...
def render_leaflet_overview(flags_df: pd.DataFrame, enriched_df: pd.DataFrame, height_px: int = 520):
    ...
    for (la1, lo1, la2, lo2) in lines:
        folium.PolyLine(locations=[[la1, lo1], [la2, lo2]], weight=3, opacity=1.0, color="#ff0000").add_to(m)
...
# ------------------- UI -------------------
st.title("RapidBus Bunching Detection by UniKL")
...
if df_raw is None or df_raw.empty:
    ...
    st.markdown("---")
    st.subheader("Bunching Detected (click on Inspect for detailed info)")
...
if not sel_vid:
    ...
    if a is None:
        ...
        # Build labels
        a_label = (a.get("bus_plate") or a.get("vehicle_id") or "–")
        b_plate = (b.get("bus_plate") if b is not None else None) or (b.get("vehicle_id") if b is not None else None) or "–"

        # Five metric columns: A, Route, Risk, Separation, B
        c1, c2, c3, c4, c5 = st.columns(5)
        c1.metric("Vehicle A", str(a_label))
        ...
        c5.metric("Vehicle B", str(b_plate))

	# Compute timestamp headway (Δtime) between A and B
	dt_label = "–"
	ta = a.get("timestamp")
	...
	if ta and tb:
	    try:
	        ...
	        m, s = divmod(dt, 60)
	        dt_label = f"{m}m {s}s" if m else f"{s}s"
	    except Exception:
	        pass

       st.caption(
    f"Separation mode: {'PATH (along route)' if str(a.get('sep_mode'))=='PATH' else 'STRAIGHT-LINE'}"
    f" | Δtime: {dt_label}"
)
